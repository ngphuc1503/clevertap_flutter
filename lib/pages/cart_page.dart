...
class Cartpage extends StatelessWidget {
  const Cartpage({super.key});
    ...
    void _purchase(BuildContext context) {
      ...
      final List<Map<String, dynamic>> itemData = Cart.items.map((item) => {
      ...
      for (final item in Cart.items) {
        item.product.stock = (item.product.stock - item.qty).clamp(0, double.infinity).toInt();
      }
          ...
          : ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  subtitle: Text('Category: ${c.product.category}  •  Qty: ${c.qty}'),
                  trailing: Text('\$${(c.qty*c.product.price).toStringAsFixed(2)}'),
                );
              },
            ),
